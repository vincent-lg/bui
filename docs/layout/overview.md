...
- [Layout](./overview.md): the window design (or how it should appear at the screen) is clearly separate from the rest, in particular from the code.  Although it is possible, programtically, to create [dynamic layouts](./dynamic.md) on the fly, the task of designing it often better done in virtual isolation from the rest, by people who are better at designing.
...
BUI is designed to strongly separate the layout from anything else.  Window layout can be described in [code](./codemd), but it is recommended to always keep it in a separate [file](./file.md).
...
Positioning everything if a grid, with each widget in its cell, would make for small widgets that might be very hard to read.  BUI will try to guess a real size (in pixels) based on the user screen and the available room in the grid.  However, it iw useful to indicate that a widget should display on 3 cells, or take 2 columns.  To do so, use the `width` and `height` attribute of every widget:
...
However, you can also write the layout in the [code](./code.md) itself.  Although this is not recommended, separating design and controls might not always be necessary and this does simplify some workflows.  It all depends on how you work, or intend to work.

Additionally, you have the option to generate layout [dynamically](./dynamic.md).  This often happens if, for instance, the user clicks on a button and you want to show a dialog.  Again, when this happens, you are encouraged to keep the dialog layout in a separate [file](./file.md), but you can also keep the layout in a Python str and ask BUI to render it.  Again, what you will choose strongly depend on how you work.

## Layout description

The examples above show that the layout should contain at least a [window](./markup/window.md) markup, containing all widgets 9graphical elements).  In fact, a layout can contain several other things:
...
1. First, if applicable, the [menubar](./markup/menubar.md) markup.  You should leave a blank line between it and the rest, for clarity, though that's far from mandatory.
...
3. Finally, the [window](/markup/window.md) definition.  This is probably where you will spend most time in terms of layout.
